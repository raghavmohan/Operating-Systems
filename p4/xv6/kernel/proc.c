...
//static struct proc* allocproc2(void);
int nextpid = 1;
...
fork(void)
{
  ...
  *np->tf = *proc->tf;
  ...
  //what to do down here: set up the new user stack
  //and some registers (np->tf->eip), 
  //np->tf->esp
...
int clone(void(*fcn)(void*), void *arg, void*stack)
{
  ...
  *(uint*)stackArg = (uint)arg;
  ...
  if(DEBUG){
    cprintf("After assign\n");
    cprintf("esp: %p\n", np->tf->esp);
    cprintf("Value at esp: %d\n",* (uint *)np->tf->esp);
    cprintf("eip: %p\n", np->tf->eip);
    cprintf("ebp: %p\n", np->tf->ebp);
  }
